...
async function setCurrentUser(spotifyApi, data) {
  ...
  spotifyApi.getMe()
        .then(async function (data) {
          ...
          if (user.length === 0) {
            createUser(data.body.id, data.body.display_name, `${data.body.images.length === null ? "" : data.body.images[0].url}`)
          } else {
...
async function updateUser(username) {
    return await User.findOneAndUpdate({ username: username }, { userIsActive: true })
}
