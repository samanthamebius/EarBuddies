...
export default function StudioCard({isListening, profileImages, profileStatus}) {
    return(
       <div className={styles.listenersImages}>
            {Array.isArray(profileImages) ? profileImages.map((listenerImage, i) => (
                <img className={styles.listenerImage} src={listenerImage} style={setListenerImageStyles(i, isListening, profileStatus)}/>
            )) : null}
        </div> 
    )    
}